 ...
 */
package org.dy.irregularlayoutview.largerimage.factory;
...
import org.dy.irregularlayoutview.largerimage.TaskQueue;
...
public class BlockImageLoader {
    ...
    public BlockImageLoader(Context context) {
        ...
        if (BASE_BLOCKSIZE <= 0) {
            ...
            BASE_BLOCKSIZE = (metrics.heightPixels + metrics.widthPixels) / 4 + (((metrics.heightPixels + metrics.widthPixels) % 4) == 0
                    ? 2 : 1);
        }
    }
    ...
    public void loadImageBlocks(List<DrawData> drawDataList, float imageScale, Rect showImageRect, int viewWidth, int viewHeight) {
        ...
        if (loadData.thumbnailBlockData.bitmap == null) {
            if (isUnRunning(loadData.thumbnailBlockData.task)) {
                loadData.thumbnailBlockData.task = new LoadThumbnailTask(loadData, decoder, thumbnailScale, imageWidth, imageHeight,
                        onImageLoadListener, onLoadStateChangeListener);
		...
		*/
        ...
        for (int row = startRow; row < endRow; row++) {
            for (int col = startCol; col < endCol; col++) {
                ...
                //blockData可能为null
                BlockData blockData = addRequestBlock(positionKey, tempCurrentDataMap.get(positionKey), loadData.currentScaleDataMap,
                        scale, imageWidth, imageHeight, decoder);
                ...
                if (bitmap != null) {
                    ...
                    if (DEBUG)
                        Log.d(TAG, "cache add--  添加  normal position :" + positionKey + " src:" + drawData.srcRect + " imageRect:" +
                                drawData.imageRect + " w:" + drawData.imageRect.width() + " h:" + drawData.imageRect.height());
                } else {
        ...
        for (int row = cacheStartRow; row < startRow; row++) {
            for (int col = cacheStartCol; col < cacheEndCol; col++) {
                positionKey.set(row, col);
                addRequestBlock(positionKey, tempCurrentDataMap.get(positionKey), loadData.currentScaleDataMap, scale, imageWidth,
                        imageHeight, decoder);
            }
        }
        // 下 #########
        ...
        if (DEBUG) {
            Log.d(TAG, "detail current scale:" + scale + " startRow:" + startRow + " endRow:" + endRow + " startCol:" + startCol + " " +
                    "endCol:" + endCol + " blockSize:" + blockSize + " size:" + loadData.currentScaleDataMap.size() + " small size:" +
                    (loadData.smallDataMap == null ? "null" : loadData.smallDataMap.size()));
            Log.d(TAG, "detail thumbnailScale:" + thumbnailScale + " cacheStartRow:" + cacheStartRow + " cacheEndRow:" + cacheEndRow + " " +
                    "cacheStartCol:" + cacheStartCol + " cacheEndCol:" + cacheEndCol + " draDataList.size:" + drawDataList.size());
...
//        if(blockData1.size()>0)
    ...
    private List<DrawData> loadSmallDatas(LoadData loadData, int scale, List<Position> needShowPositions, int startRow, int endRow, int
            startCol, int endCol) {
        ...
        if (loadData.smallDataMap != null && !loadData.smallDataMap.isEmpty()) {
            ...
            while (blockIterator.hasNext()) {
                ...
                if (blockData.bitmap != null && position.row >= startRowKey && position.row <= endRowKey && position.col >= startColKey
                        && position.col <= endColKey) {
                    ...
                    for (int row = startPositionRow, i = 0; row < endPositionRow; row++, i++) {
                        ...
                        for (int col = startPositionCol, j = 0; col < endPositionCol; col++, j++) {
                            ...
                            if (needShowPositions.remove(currentScalePosition.set(row, col))) {
                                ...
                                if (DEBUG) {
                                    Log.d(TAG, "cache add--添加  smallDataMap position :" + position + " 到 当前currentScalePosition:" +
                                            currentScalePosition + " src:" + drawData.srcRect + "w:" + drawData.srcRect.width() + " h:" +
                                            drawData.srcRect.height() + " imageRect:" + drawData.imageRect + " w:" + drawData.imageRect
                                            .width() + " h:" + drawData.imageRect.height());
                                }
                            }
                        }
                    }
    ...
    private BlockData addRequestBlock(Position positionKey, BlockData blockData, Map<Position, BlockData> currentDataMap, int scale, int
            imageWidth, int imageHeight, BitmapRegionDecoder decoder) {
        if (blockData == null) {
        ...
        if (blockData.bitmap == null && isUnRunning(blockData.task)) {
            blockData.task = new LoadBlockTask(blockData.position, blockData, scale, imageWidth, imageHeight, decoder,
                    onImageLoadListener, onLoadStateChangeListener);
    ...
    private static class LoadBlockTask extends TaskQueue.Task {
        ...
        LoadBlockTask(Position position, BlockData blockData, int scale, int imageWidth, int imageHeight, BitmapRegionDecoder decoder,
                      OnImageLoadListener onImageLoadListener, OnLoadStateChangeListener onLoadStateChangeListener) {
        ...
        protected void onPostExecute() {
            super.onPostExecute();
            if (DEBUG) {
                Log.d(TAG, "finish LoadBlockTask position:" + position + " currentScale:" + scale + " bitmap: " + (bitmap == null ? "" :
                        bitmap.getWidth() + " bitH:" + bitmap.getHeight()));
            }
            blockData.task = null;
            if (bitmap != null) {
    ...
    private static class LoadThumbnailTask extends TaskQueue.Task {
        ...
        LoadThumbnailTask(LoadData loadData, BitmapRegionDecoder decoder, int thumbnailScale, int imageWidth, int imageHeight,
                          OnImageLoadListener onImageLoadListener, OnLoadStateChangeListener onLoadStateChangeListener) {
        ...
        protected void onPostExecute() {
            super.onPostExecute();
            if (DEBUG) {
                Log.d(TAG, "LoadThumbnailTask bitmap:" + bitmap + " currentScale:" + scale + " bitW:" + (bitmap == null ? "" : bitmap
                        .getWidth() + " bitH:" + bitmap.getHeight()));
            }
